...
public class LinkedList<Type> {
    @FunctionalInterface
    public interface SearchFunction<Type> {
        boolean condition(Type element);
    }
    ...
    public Node<Type> search(SearchFunction<Type> searchFunction) {
        Node<Type> searchedNode = head;
        while (searchedNode != null) {
            if (searchFunction.condition(searchedNode.getElement())) { break; }
    ...
    public boolean insertBeforeElement(SearchFunction<Type> searchFunction, Type newElement) {
    ...
    public boolean insertAfterElement(SearchFunction<Type> searchFunction, Type newElement) {
        Node<Type> searchedNode = search(searchFunction);
        if (searchedNode == null) { return false; }
    ...
    public <R> R fold(R initial, BiFunction<R, Type, R> accumulator) {
        R result = initial;
        Node<Type> curr = head;
        while (curr != null) {
            result = accumulator.apply(result, curr.getElement());
            curr = curr.getNext();
        }
    ...
    public void forEach(Function<Type, Void> action) {
        Node<Type> curr = head;
        while (curr != null) {
            action.apply(curr.getElement());
            curr = curr.getNext();
        }
    }
}
